...
def train_network(config):
    ...
    if not config.dynamic_learning_params:
        ...
        optimizer_scdualer = ReduceLROnPlateau(optimizer, 'min', patience=config.lr_patience_factor * config.accumulate_loss_batch_factor, factor=config.lr_decay_factor,cooldown=10,min_lr=1e-7)
    else:
