...
class SkipConnections(nn.Module):
    def __init__(self, channel_input_number, channel_output_number):
        super().__init__()
        self.model_skip_connections_inter= nn.Sequential(nn.Conv1d(channel_input_number,channel_output_number, 1),nn.BatchNorm1d(channel_output_number))
    def forward(self,x):
...
class BranchLeafBlockSkipConnections(BranchLeafBlock):
    ...
    def forward(self, x):
        out = BranchLeafBlock.__call__(self,x)+self.skip_connections.forward(x)
        return out
...
class IntersectionBlockSkipConnections(IntersectionBlock):
    ...
    def forward(self, x):
        out = IntersectionBlock.__call__(self,x) + self.skip_connections.forward(x)
        return out
...
class BranchBlockSkipConnections(BranchBlock):
    ...
    def forward(self, x,prev_segment):
        out = BranchBlock.__call__(self,x,prev_segment)+self.skip_connections(torch.cat([prev_segment, x], dim=SYNAPSE_DIMENTION_POSITION))
        return out
...
class RootBlockSkipConnections(RootBlock):
    ...
    def forward(self, x):
        return RootBlock.__call__(self,x)
