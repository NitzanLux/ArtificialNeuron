...
from keras.layers import Conv2D, MaxPooling2D, AveragePooling2D, Conv1D, Cropping1D, UpSampling1D, MaxPooling1D, \
    AveragePooling1D
...
def create_temporaly_convolutional_model(max_input_window_size, num_segments, num_syn_types, num_DVT_outputs,
    ...
    for k in range(len(filter_sizes_per_layer)):
        ...
        if k == 0:
            x = Conv1D(num_filters, filter_size, activation=activation, kernel_initializer=initializer,
                       kernel_regularizer=l2(l2_reg),
                       strides=stride, dilation_rate=dilation_rate, padding='causal', name='layer_%d' % (k + 1))(
                binary_input_mat)
        else:
            x = Conv1D(num_filters, filter_size, activation=activation, kernel_initializer=initializer,
                       kernel_regularizer=l2(l2_reg),
    ...
    output_spike_predictions = Conv1D(1, 1, activation='sigmoid', kernel_initializer=output_spike_init_weights,
                                      bias_initializer=output_spike_init_bias,
                                      kernel_regularizer=l2(1e-8), padding='causal', name='spikes')(x)
    output_soma_voltage_pred = Conv1D(1, 1, activation='linear', kernel_initializer=output_soma_init,
                                      kernel_regularizer=l2(1e-8), padding='causal', name='somatic')(x)
    output_dend_voltage_pred = Conv1D(num_DVT_outputs, 1, activation='linear', kernel_initializer=output_dend_init,
                                      kernel_regularizer=l2(1e-8), padding='causal', name='dendritic')(x)
...
temporal_conv_net = create_temporaly_convolutional_model(input_window_size, num_segments, num_syn_types,
                                                         num_DVT_components,
...
for learning_schedule in range(start_learning_schedule, num_learning_schedules):
    ...
    temporal_conv_net.compile(optimizer=optimizer_to_use, loss=['binary_crossentropy', 'mse', 'mse'],
