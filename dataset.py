...
NUM_SIMULATIONS = 128
NUM_SIMULATIONS = 1
TOTAL_SIM_DURATION_IN_SEC = 6
TOTAL_SIM_DURATION_IN_SEC = 2

# switch whether to store dendritic voltage traces (DVTs), which take up a lot of storage
COLLECT_AND_SAVE_D_V_TS = True

# high res sampling of the voltage and nexus voltages
NUM_SAMPLES_PER_M_S_HIGH_RES = 8

# synapse type
EXCITATORY_SYNAPSE_TYPE = 'NMDA'    # supported options: {'AMPA','NMDA'}
# excitatorySynapseType = 'AMPA'    # supported options: {'AMPA','NMDA'}
INHIBITORY_SYNAPSE_TYPE = 'GABA_A'
...
# simulation duration
SIM_DURATION_SEC = TOTAL_SIM_DURATION_IN_SEC
...
# NMDA
NUM_BAS_EX_SPIKES_PER100MS_RANGE = [0, 800]
NUM_BAS_EX_INH_SPIKE_DIFF_PER100MS_RANGE = [-600, 200]
NUM_APIC_EX_SPIKES_PER100MS_RANGE = [0, 800]
NUM_APIC_EX_INH_SPIKE_DIFF_PER100MS_RANGE = [-600, 200]
...
# beaurrocracy
SHOW_PLOTS = False
RESULTS_SAVED_IN_ROOT_FOLDER = '/david.beniaguev/Reseach/Single_Neuron_InOut/ExperimentalData/'

USE_CVODE = True
TOTAL_SIM_DURATION_IN_M_S = 1000 * TOTAL_SIM_DURATION_IN_SEC
...
def GetDirNameAndFileName(numOutputSpikes, randomSeed):
    ...
    cellType = 'L5PC'
    synapseTypes    = EXCITATORY_SYNAPSE_TYPE + '_' + INHIBITORY_SYNAPSE_TYPE
    dendritesKind = 'activeDendrites'
    if not USE_ACTIVE_DENDRITES:
        dendritesKind = 'passiveDendrites'
    else:
        dendritesKind += '_Ih_vshift_%d_SKE2_mult_%d' %(IH_VSHIFT, 100 * S_K_E2_MULT_FACTOR)

    modelString = cellType + '__' + dendritesKind + '__' + synapseTypes
    dirToSaveIn = RESULTS_SAVED_IN_ROOT_FOLDER + modelString + '//'

    # string to describe input
    string1 = 'exBas_%d_%d_inhBasDiff_%d_%d' %(NUM_BAS_EX_SPIKES_PER100MS_RANGE[0], NUM_BAS_EX_SPIKES_PER100MS_RANGE[1],
                                               NUM_BAS_EX_INH_SPIKE_DIFF_PER100MS_RANGE[0], NUM_BAS_EX_INH_SPIKE_DIFF_PER100MS_RANGE[1])
    string2 = 'exApic_%d_%d_inhApicDiff_%d_%d' %(NUM_APIC_EX_SPIKES_PER100MS_RANGE[0], NUM_APIC_EX_SPIKES_PER100MS_RANGE[1],
                                                 NUM_APIC_EX_INH_SPIKE_DIFF_PER100MS_RANGE[0], NUM_APIC_EX_INH_SPIKE_DIFF_PER100MS_RANGE[1])
    ...
    simulationString = string3 %(savedDVTs, numOutputSpikes, NUM_SIMULATIONS, TOTAL_SIM_DURATION_IN_SEC, randomSeed)
...
for section in all_sections_with_SKE2:
    ...
    new_SKE2_g = orig_SKE2_g * S_K_E2_MULT_FACTOR
...
print('-------------------------------------/')
for simInd in range(NUM_SIMULATIONS):
    ...
    ex_spikes_bin, inh_spikes_bin = generate_input_spike_trains_for_simulation(SIM_DURATION_MS,
                                                                               ...
                                                                               apical_seg_length_um,
                                                                               MIN_SEG_LENGTH_UM,
                                                                               NUM_BAS_EX_SPIKES_PER100MS_RANGE,
                                                                               NUM_APIC_EX_SPIKES_PER100MS_RANGE,
                                                                               NUM_BAS_EX_INH_SPIKE_DIFF_PER100MS_RANGE,
                                                                               NUM_APIC_EX_INH_SPIKE_DIFF_PER100MS_RANGE,
    ...
    for segInd, segment in enumerate(allSegments):
        ...
        if EXCITATORY_SYNAPSE_TYPE == 'AMPA':
            exSynapse = DefineSynapse_AMPA(segment)
        elif EXCITATORY_SYNAPSE_TYPE == 'NMDA':
        ...
        # define synapse and connect it to a segment
        if INHIBITORY_SYNAPSE_TYPE == 'GABA_A':
            inhSynapse = DefineSynapse_GABA_A(segment)
        elif INHIBITORY_SYNAPSE_TYPE == 'GABA_B':
            inhSynapse = DefineSynapse_GABA_B(segment)
        elif INHIBITORY_SYNAPSE_TYPE == 'GABA_AB':
...
# %% simulate the cell
    ...
    h.finitialize(-76)
    neuron.run(TOTAL_SIM_DURATION_IN_M_S)
    ...
    recordingTimeHighRes = np.arange(0, TOTAL_SIM_DURATION_IN_M_S, 1.0 / NUM_SAMPLES_PER_M_S_HIGH_RES)
    ...
    recordingTimeLowRes = np.arange(0, TOTAL_SIM_DURATION_IN_M_S)
    ...
    # show the results
    if COLLECT_AND_SAVE_D_V_TS and SHOW_PLOTS:
        ...
        plt.xlim(0, TOTAL_SIM_DURATION_IN_M_S)
...
totalNumSimulationSeconds = TOTAL_SIM_DURATION_IN_SEC * NUM_SIMULATIONS
...
experimentParams['numSimulations'] = NUM_SIMULATIONS
experimentParams['totalSimDurationInSec']   = TOTAL_SIM_DURATION_IN_SEC
experimentParams['collectAndSaveDVTs']      = COLLECT_AND_SAVE_D_V_TS
experimentParams['numSamplesPerMS_HighRes'] = NUM_SAMPLES_PER_M_S_HIGH_RES
experimentParams['excitatorySynapseType']   = EXCITATORY_SYNAPSE_TYPE
experimentParams['inhibitorySynapseType']   = INHIBITORY_SYNAPSE_TYPE
experimentParams['useActiveDendrites']      = USE_ACTIVE_DENDRITES
...
experimentParams['num_bas_ex_spikes_per_100ms_range']          = NUM_BAS_EX_SPIKES_PER100MS_RANGE
experimentParams['num_bas_ex_inh_spike_diff_per_100ms_range']  = NUM_BAS_EX_INH_SPIKE_DIFF_PER100MS_RANGE
experimentParams['num_apic_ex_spikes_per_100ms_range']         = NUM_APIC_EX_SPIKES_PER100MS_RANGE
experimentParams['num_apic_ex_inh_spike_diff_per_100ms_range'] = NUM_APIC_EX_INH_SPIKE_DIFF_PER100MS_RANGE
