...
class BranchLeafBlock(SegmentNetwork):
    def __init__(self, input_shape: Tuple[int, int], channel_input, channels_number, channel_output, kernel_size_2d,
        ...
        padding_factor = self.keep_dimensions_by_padding_claculator(input_shape, (kernel_size_1d, 1), stride, dilation)
...
class BranchBlock(SegmentNetwork):
    def __init__(self, input_shape: Tuple[int, int], channel_input, channels_number, channel_output, kernel_size_2d,
        ...
        self.conv2d_prev = weight_norm(
            nn.Conv2d(channels_number, channels_number, kernel_size_2d,  # todo: weight_norm???
                      stride=stride, padding=padding_factor, dilation=dilation))
        ...
        self.conv1d = weight_norm(nn.Conv2d(channels_number, channel_output, (kernel_size_1d, 1),
                                            stride=stride, padding=padding_factor,
                                            dilation=dilation))
        self.init_weights()
...
class IntersectionBlock(SegmentNetwork):
    def __init__(self, input_shape: Tuple[int, int], channel_input, channels_number, channel_output, kernel_size_2d,
        ...
        self.batch_normalization = torch.nn.BatchNorm2d(channels_number)
        self.conv1d_2 = weight_norm(nn.Conv2d(channels_number, channel_output, (kernel_size_1d, 1),
                                              stride=stride, padding=padding_factor,
                                              dilation=dilation))
    ...
    def forward(self, x):
        ...
        for node in self.segment_tree:
            if node.type == SectionType.BRANCH_LEAF:
                ...
                indexs = node.get_prev_node_representative()
                input = [representative_dict[out] for out in indexs]
