...
for i in range(len(layers_original)):
    plt.annotate(
        ...
        (layers_original[i] + 0.1, original_auc_plotting[i] + (0.002 * ((i == 0) * 2 - 1)) * ((i == 0) + 1)),
...
for i in range(len(layers_reduction)):
    plt.annotate(
        ...
        (layers_reduction[i] + 0.1, reduction_auc_plotting[i] - (0.002 * ((i == 0) * 2 - 1)) * ((i == 0) + 1)),
...
ann = plt.annotate(text,
                  xy=(0.5, 0.1),xycoords='axes fraction',textcoords='axes fraction',size=15,
