...
for i in range(len(layers_reduction)):
    plt.annotate(human_format(batch_counter_reduction_mean[i]) +r" $\pm$ " + human_format(batch_counter_reduction_std[i]), (layers_reduction[i]+0.1,reduction_auc_plotting[i]+0.001), fontsize=FONT_SIZE,color=(0/255., 90/255., 216/255.))
plt.legend()
plt.show()
