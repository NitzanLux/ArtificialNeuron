...
for i in range(len(layers_original)):
    ...
    plt.annotate(str(human_format(batch_counter_original_mean[i])+r" $\pm$ "+human_format(batch_counter_original_std[i])), (layers_original[i]+0.1,original_auc_plotting[i]+(0.0005*((i==0)*2-1))*((i==0)*2)), fontsize=FONT_SIZE,color=(255/255., 57/255., 70/255.))
