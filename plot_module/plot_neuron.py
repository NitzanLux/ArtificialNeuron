...
def plot_morphology_from_cell(ax, cell,spread_dend=False,remove_axon=True,segment_colors=None, width_mult_factors=None, colors_dict={},
    ...
    for ind, key in enumerate(data_dict.keys()):
        ...
        if "axon" in curr_data['what']  and remove_axon:
            continue
        part_name = ((" " + curr_data['what'][:1] if "axon" in curr_data['what'] else " axon") if not color_by_type else "")
        ...
        if spread_dend:
            ...
            data_x.extend(np.sin((ind)*2*np.pi/angle_counter)*np.sqrt(np.abs(cur_x)))
            data_y.extend(np.cos((ind)*2*np.pi/angle_counter)*np.sqrt(np.abs(cur_y)))
