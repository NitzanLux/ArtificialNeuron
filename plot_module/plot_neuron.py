...
def map_cell_to_xyzd(cell,approximate_xyz=False):
    from neuron import h
    # apic=cell.apic
    # dend = cell.dend
    # soma = cell.soma
    # axon = cell.axon
    section_list_by_morph = [cell.apic, cell.apic,cell.soma,cell.axon]
    ...
    empty_xyz_sec=[]
    if approximate_xyz:
        for what, sections_list in zip(["apical", "basal", "soma", "axon"],section_list_by_morph):
            empty_xyz_sec.append([])
            for sec_ind, sec in enumerate(sections_list):
                p = sec.psection()['morphology']['pts3d']
                if len(p)==0:
                    empty_xyz_sec[-1].append(np.linspace(0,sec.L,sec.nseg))
                else:
                    empty_xyz_sec[-1].append([])
    counter=0
    for what, sections_list in zip(["apical", "basal", "soma", "axon"],
        ...
        for sec_ind, sec in enumerate(sections_list):
            ...
            if len(p)==0 and approximate_xyz:
                y_path = soma[1]+np.cos((counter*np.pi)/len(empty_xyz_sec))*np.sqrt(empty_xyz_sec[counter][sec_counter])
                x_path =  soma[0]+np.sin((counter*np.pi)/len(empty_xyz_sec))*np.sqrt(empty_xyz_sec[counter][sec_counter])
                z_path = soma[3]+[0]*empty_xyz_sec[counter][sec_counter].size
...
def plot_morphology_from_cell(ax, cell, approximate_xyz=False,segment_colors=None, width_mult_factors=None, colors_dict={},
    ...
    soma_mean, all_section_coords, all_segment_coords = map_cell_to_xyzd(cell,approximate_xyz=approximate_xyz)
    ...
    for ind, key in enumerate(data_dict.keys()):
        ...
        # if approximate_xyz:
        #     data_x.extend(curr_data)
        #     data_y.extend(curr_data)
        #     data_z.extend(curr_data)
        # else:
    ...
    for i,start in enumerate(steps_seg[:-2]):
        start = max(0,start)
        ax.plot(data_arr[start:steps_seg[i+1],1], data_arr[start:steps_seg[i+1],0], lw=seg_width[i],color = new_color[i])
    ax.axis('scaled')
...
from neuron import h
fig,ax = plt.subplots()
plot_morphology_from_cell(ax,l5pc,approximate_xyz=True)
