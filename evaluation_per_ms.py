...
import simulation_data_generator
def plot_network_and_actual_results(file_path: [str, List[str]], model_path: [str, List[str]] = '',
                                    ...
                                    window_size: int = 2000, include_DVT=False, DVT_PCA_model=None):
    # if include_DVT:
    ...
    data_generator = simulation_data_generator.SimulationDataGenerator([file_path],1,is_shuffle_data=False,shuffle_files=False)
    ...
    for p in model_path:
        try:
            ...
            network = recursive_neuronal_model.RecursiveNeuronModel.load(p)
        # network.cpu()
        ...
        out_var = []
        spike = []
        for i in tqdm(range(network.time_domain_shape, window_size)):
            x,out = data_generator[sample_idx,i+time_idx]
            s,v=out
            out = network(x)
        ...
        axs[0].plot(np.arange(network.time_domain_shape, window_size), out_var, label=model_id)
        axs[1].plot(np.arange(network.time_domain_shape, window_size), spike, label=model_id)
    sim_inds=sample_idx*np.ones((window_size,)).astype(int)
    time_inds=np.arange(time_idx,time_idx+window_size)
    x,out = data_generator[sim_inds,time_inds]
    spikes, v=out
    for s in np.where(spikes == 1)[0]:
        print(s)
        axs[1].axvline(s, 0, 1, color='red')
    axs[1].set_xlim([0, v.shape[0]])
    axs[0].set_ylim([-80, -54])

    axs[0].plot(v, label='original')
