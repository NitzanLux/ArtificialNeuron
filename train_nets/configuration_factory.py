...
def change_configs_in_json(configs_json,update_funnction=None,**kwargs):
...
def restore_configs_from_temp(configs_json):
    ...
    for conf in configs:
        conf[-1]=conf[-1]+'temp'
        path = os.path.join(MODELS_DIR, *conf)
        if not os.path.exists(path[:-len('temp')]) or os.path.getsize(path[:-len('temp')])==0:
        ...
        for i in range(7, 0, -2):
            ...
            for data in [DAVID_BASE_PATH, REDUCTION_BASE_PATH]:
                config = config_factory(
                    ...
                    accumulate_loss_batch_factor=1, prediction_length=700,torch_seed=torch_seed,numpy_seed=numpy_seed,random_seed=random_seed,input_window_size=120,
